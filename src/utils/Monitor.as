...
//} endregion
		...
		private var _bitmapData:BitmapData = new BitmapData(DEFAULT_CHART_WIDTH, DEFAULT_CHART_HEIGHT, true, 0);
		...
		public function Monitor(myUpdateRate:Number = DEFAULT_UPDATE_RATE)
		{
			...
			_style.setStyle("monitor", {fontSize: "9px", fontFamily: "_sans", leading: "-2px"});
...
//addEventListener(Event.REMOVED_FROM_STAGE, removedFromStageHandler);
		...
		public function setChartSize(width:int, height:int):void
		{
			...
			bmp.copyPixels(_bitmapData, new Rectangle(0, 0, width, height), new Point(width - _bitmapData.width, height - _bitmapData.height));
...
// prepare bitmap data
				...
				_bitmapData.fillRect(new Rectangle(0, 0, 1, _bitmapData.height), 0);
				_bitmapData.lock();
				
				_xml.framerate = "framerate: " + _framerate + " / " + stage.frameRate;
				_bitmapData.setPixel32(0, (1. - _framerate / stage.frameRate) * (_bitmapData.height - 1), 0xff000000 | _colors["framerate"])
...
// memory
				...
				_bitmapData.setPixel32(0, (1. - totalMemory / _maxMemory) * (_bitmapData.height - 1), 0xff000000 | _colors["memory"])
...
// properties
				for (var target:Object in _targets)
				{
					...
					for (var i:int = 0; i < numProperties; ++i)
					{
						...
						if ((scale = _scales[property]) != 0.)
						{
							...
							_bitmapData.setPixel32(0, (1. - scaledValue) * (_bitmapData.height - 1), 0xff000000 | _colors[property]);
						}
					}
				}
		 ...
		 */
		public function watchProperty(target:Object, property:String, color:int = 0, scale:Number = 0., overflow:Boolean = false):Monitor
		{
			if (!_targets[target])
		...
		public function watch(target:Object, properties:Array, colors:Array = null, scales:Array = null, overflows:Array = null):Monitor
		{
			var numProperties:int = properties.length;
			
			for (var i:int = 0; i < numProperties; ++i)
			{
				watchProperty(target, properties[i], colors ? colors[i] : _defaultColor, scales && scales[i] ? scales[i] : 0., overflows ? overflows[i] : false);
			}
